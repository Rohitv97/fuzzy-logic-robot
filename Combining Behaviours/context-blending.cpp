...
pair<double, double> wall_follow(double rfs, double rbs)
{
	Member m1;
	m1.memberName = "L";
...
pair<double, double> obstacle_avoid(double lfs, double fs, double rfs)
{
...
pair<double, double> context_blending(double dist, pair<double, double> ref, pair<double, double> oa)
{
	Member m1;
	m1.memberName = "OA";
	...
	fLspeed = ((f_ref * ref.first) + (f_oa * oa.first)) / (f_ref + f_oa);

	fRspeed = ((f_ref * ref.second) + (f_oa * oa.second)) / (f_ref + f_oa);
...
int main(int argc, char **argv)
{
	...
	while(true)
	{
		...
		comb_res = context_blending(dist, ref, oa);

		//Setting velocity
		
		robot.setVel2(comb_res.first, comb_res.second);
		cout << "Left, Right ----->  " << comb_res.first << "," << comb_res.second << endl;
